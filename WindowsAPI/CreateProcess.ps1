...
function Invoke-NewProces {  
    ...
    # Call CreateProcess
    [Kernel32]::CreateProcess("C:\Windows\System32\powershell.exe", "C:\Windows\System32\calc.exe", [ref] $SecAttr, [ref] $SecAttr, 0x1,
    0x1, [IntPtr]::Zero, $GetCurrentPath, [ref] $StartupInfo, [ref] $ProcessInfo) |out-null
 
 }

#The flags which were set above should create a "cmd.exe" process that has no window, which in turn launches calc. In fact you can
